...
/**
 * 1284. 수도 요금 경쟁
 * https://swexpertacademy.com/main/code/problem/problemDetail.do?problemLevel=2&contestProbId=AV189xUaI8UCFAZN&categoryId=AV189xUaI8UCFAZN&categoryType=CODE&problemTitle=&orderBy=PASS_RATE&selectCodeLang=JAVA&select-1=2&pageSize=30&pageIndex=1
 *
...
public class SWEA_D2_1284 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < T; i++) {
            int a = getA(arr[i][0], arr[i][4]);
            int b = getB(arr[i][1], arr[i][2], arr[i][3], arr[i][4]);
            System.out.println("#" + (i + 1) + " " + Math.min(a, b));
        }
    }

    // A사 수도 요금 계산
    public static int getA(int p, int w) {
        int charge = 0;
    ...
    // B사 수도 요금 계산
    public static int getB(int q, int r, int s, int w) {
        int charge = 0;

        // 기본 요금이 Q원이고, 월간 사용량이 R리터 이하인 경우 요금은 기본 요금만 청구된다.
